# client/desktop

这个目录放 LinkNest 的桌面端代码。当前目标是提供一个 Windows GUI，把现有 CLI 的认证、设备、文件和任务能力封装成可视化界面。

当前实现基于 Fyne，桌面端主要复用 `client/internal/*` 里的现有 Go 逻辑，而不是重新实现一套 HTTP 或 CLI 包装层。当前桌面端已经包含账号、设备、文件和上传任务四个页签，并支持后台操作状态提示、自动刷新和可停止的在线心跳。共享服务层位于 `client/internal/appsvc/`，供桌面端和移动端共同使用。

Windows 原生构建说明：

- 推荐先创建输出目录：`mkdir -p ./bin`
- 入口：`go build -o ./bin/linknest-desktop.exe ./client/desktop/cmd/linknest-desktop`
- 由于 Fyne 的 Windows 图形栈依赖 `CGO`，本机需要可用的 `gcc`/`g++` 工具链
- 推荐使用 MSYS2 UCRT64 的 `gcc` / `g++`
- 如果本机只有 Go 而没有 C 编译器，WSL 侧 `go test ./...` 仍然可以通过，但 Windows GUI 可执行文件无法直接编出来
- 在 WSL/Linux 交叉编译 Windows GUI 时，需要 Linux 侧 `mingw-w64`，并通过 `GOOS=windows GOARCH=amd64 CGO_ENABLED=1 CC=x86_64-w64-mingw32-gcc-posix` 构建；也可以直接运行仓库根目录的 `scripts/build-gui-artifacts.sh`。

目录结构：

- `cmd/`：桌面应用入口。
- `internal/`：桌面端界面实现。
